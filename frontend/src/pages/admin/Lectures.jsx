  ...
  const handleSaveLecture = async () => {
    try {
      ...
      if (
        ...
        alert(
          "Please fill all required fields: Teacher ID, Course ID, Instructor, Semester, and Credit Hour."
        );
        return;
      }
      ...
      if (currentLecture._id) {
        res = await axios.put(
          `http://localhost:5000/api/lectures/${currentLecture._id}`,
          currentLecture
        );
        console.log("Update Response:", res.data);
        setLectures((prev) =>
          prev.map((lec) => (lec._id === res.data._id ? res.data : lec))
        );
      } else {
      ...
      alert(
        "Failed to save lecture: " + (err.response?.data?.error || err.message)
      );
    }
  };
  ...
  return (
    <Box>
              ...
              {loading ? (
                ...
                lectures.map((lecture) => (
                  <TableRow key={lecture._id}>
                    ...
                    <TableCell>
                      {lecture.courseName} ({lecture.courseId})
                    </TableCell>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} fullWidth>
        <DialogTitle>
          {currentLecture._id ? "Edit Lecture" : "Add Lecture"}
        </DialogTitle>
        <DialogContent>
          <TextField
            select
            ...
            InputProps={{
              endAdornment: (
                <InputAdornment position="end">
                  <IconButton
                    size="small"
                    onClick={() => handleCreditHourChange(-1)}
                  >
                    <RemoveCircle />
                  </IconButton>
                  <IconButton
